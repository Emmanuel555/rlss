...
using RLSSHardOptimizer = rlss::RLSSHardOptimizer<double, DIM>; // 1
using RLSSSoftOptimizer = rlss::RLSSSoftOptimizer<double, DIM>; // 2
using RLSSHardSoftOptimizer = rlss::RLSSHardSoftOptimizer<double, DIM>; // 3
...
int main(int argc, char* argv[]) {
    ...
    // robot params, path constraints and assignment of final targets begin here 
    for(auto& p:
        ...
        planners.emplace_back( // sets in stone all the fixed params and gains including intermediary targets written in the json files
                goal_selector,
                trajectory_optimizer,
                discrete_path_searcher,
                validity_checker,
                max_rescaling_count,
                rescaling_multiplier // duration related
        );
...
//    json_builder.addOccupancyGridToCurrentFrame(occupancy_grid);
    ...
    StdVectorVectorDIM simulated_obstacle(DIM);
    double total_displacement_0 = 0.0;
    double total_displacement_1 = 0.0;

    while(!allRobotsReachedFinalStates( // bool function at the top, while all robots have not reached their final state => while false;
...
//                std::cout << "robot " << i << " eval" << std::endl;
            ...
            std::cout << "Drone " << i << " nett euclid distance " << ((original_trajectories[i].eval(original_trajectories[i].maxParameter(), 0))-(original_trajectories[i].eval(0, 0))).norm() << std::endl;            
            std::cout << "Drone " << i << " theoretical euclidean velocity per actual duration" << ((original_trajectories[i].eval(original_trajectories[i].maxParameter(), 0))-(original_trajectories[i].eval(0, 0))).norm()/original_trajectories[i].maxParameter() << std::endl;
            std::cout << "Drone " << i << " actual required velocity per replanning period: " << (states[i][0] - old_position_state[i]).norm()/replanning_period << std::endl;
...
//        planner.statisticsStorage().save();
        ...
        std::cout << "Actual Required Velocity of Drone 0: " << total_displacement_0/current_time << std::endl;
        std::cout << "Total displacement of Drone 1: " << total_displacement_1 << std::endl;
        std::cout << "Actual Required Velocity of Drone 1: " << total_displacement_1/current_time << std::endl;
        std::cout << "num obstacles: " << occupancy_grid.size() << std::endl;
        std::cout << "\n";
    }
